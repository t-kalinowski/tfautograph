...
The R package `tfautograph` helps you write nautral R code with tensorflow. It allows you to write R code as you would naturally, which then gets translated to tensorflow equivalent control flow statements. This vignette goes through some of the main features and then goes into how it works a little. 
...
#### `for`
Autographed `for` loops build ontop of while loops. Presently, `autograph` adds support for new types of values passed to `for`: tfdatasets, tensors, and (in eager mode only) python iterators 
...
while(!is.null(elem <- iter_next(iterable))) {...}
```
...
in this Rmarkdown vignetter. (If we don't redirect output to a file, then it
