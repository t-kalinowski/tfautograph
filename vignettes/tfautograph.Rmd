...
to tensorflow, but it has very little relation to that code base. If you're
...
evaluated every time the funciton is called.
...
tfautograph will translate R control flow statements to tensorflow. This
includes `if`, `while`, `for`, `break`, `next`, and `switch`. Here is handy
summary table of the translation endpoints (note, these summary code snippits
are meant to concisely convey the spirit of the translation, not the actual
implementation)
...
knitr::kable(matrix(
  ...
  c("`if(x)`", "`tf$cond(x, ...)`", "```if(x$`__bool__`())```",
    ...
    "`for(x in tensor)`", "`tf$while_loop(...)`" , "`while(!is.null(var <- iter_next(tensor_iterator)))`",
    "`for(x in tfdataset)`", "`Dataset$reduce()`", "`while(!is.null(var <- iter_next(dataset_iterator)))`" 
  ),
))
```
...
methods for many S3 generics, including `as.logical()`, so this works as you
would expect.
...
`tfautograph::view_function_graph()` to launch tensorboard with the produced
function graph.
...
Side effects Ops `tf$print()` and `tf$Assert()` created within a `tf.function` are executed in-line and in the correct order when the funciton is evaluated. (If you're used to working Tensorflow v1, you read that right!)
