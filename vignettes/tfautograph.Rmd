...
## Compatability
tfautograph works with Tensorflow versions 1.14, 1.15, and 2.0. This vignette is
using version 2.0.
```{r}
tf$version$VERSION
```
...
these summary code snippits are meant to concisely convey the spirit of the
...
Side effects Ops `tf$print()` and `tf$Assert()` created within a `tf.function` are executed in-line and in the correct order when the funciton is evaluated. (If you're used to working Tensorflow version 1, you read that right!)
...
+  `tf_assert()`: A thin wrapper around `tf.Assert()` that automatically generates a useful error message that includes the R call stack and the values of the tensors involved in the expression.
